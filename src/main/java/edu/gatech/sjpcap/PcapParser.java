...
public class PcapParser {
    ...
    private FileInputStream file_is;
    ...
    public int openFile(String path) {
        try {
            openStreamFromFile(new FileInputStream(new File(path)));
        } catch (Exception e) {
            e.printStackTrace();
            return -1;
        }
    ...
    private int openStreamFromFile(FileInputStream file_is) {
        try {
            this.fis = new DataInputStream(file_is);
        } catch (Exception e) {
            e.printStackTrace();
            return -1;
        }
